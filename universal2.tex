...
\begin{proof}
    ...
    Suppose, therefore, that $v_1\neq v_2$ so that $v_1v_2\in E(H)$.  Recall the definition of $S^+_y$, which implies that $v_1,v_2\in S^+_y\cap S^+_{y+1}$.  Since $v_1v_2\in E(H)$, one of $v_1$ or $v_2$ is an $H$-parent of the other. Without loss of generality suppose that $v_1$ is an $H$-parent of $v_2$.  By \pref{clique-path}, one of $x_2':=\sigma_y(x_y(v_2))$ or $x_1=\sigma_y(x_y(v_1))$ is a prefix of the other.  As in the previous proof, this implies that, for a fixed $x_1$, the number of choices of $(x_2',z_1)$ is $n^{1+o(1)}/2^{|x_1|}$.

    Now, there exists a binary string $\nu'_y(v_2)$ of length $o(\log n)$ such that $J(x_2',\nu'_y(v_2))=\sigma_{y+1}(x_{y+1}(v))=x_2$.  For a fixed $x_2'$, the number of choices of $\nu'_y(v_2)$ is $2^{o(\log n)}=n^{o(1)}$ and, since $J$ is a function, the number of choices for $x_2$ is at most $n^{o(1)}$.  Therefore, for fixed $x_1,x_2',z_1$, the number of possible choices for $(x_2,z_2)$ is at most $n^{o(1)}\cdot 2^\lambda = n^{o(1)}$.

    To summarize, in any of the cases $v_1=v_2$ or $v_1\neq v_2$, for a fixed $(x_1,y_1,z_1)$, the number of choices for $(x_2,y_2,z_2)$ is $n^{1+o(1)}/2^{|x_1|}$.  We can now finish as in the previous proof by summing over the $\log n+\lambda+1$ possible values of $|x_1|$ to get an upper bound of
    \[
        \sum_{r=0}^{\log n+\lambda} 2^r\cdot n^{o(1)}\cdot n^{1+o(1)}/2^r
        = n^{1+o(1)} \enspace . \qedhere
    \]
\end{proof}
